...
import stripe
stripe.api_key = "sk_test_TZqpY1tMJBGrhxPWLVfGWcwt"
...
class BillingProfileManager(models.Manager):
	def new_or_get(self, request):
		user = request.user
		guest = request.session.get('guest_email_id')
		print(guest)
		obj=None
		created=False
		if user.is_authenticated():
			...
			guest_email_obj, created = GuestEmail.objects.get_or_create(
								email=guest)
			obj, billing_profile_created_for_guest = self.model.objects.get_or_create( 
	...
	def add_new(self,billing_profile,stripe_card_response):
		if str(stripe_card_response.object) == "card":
			new_card = self.model(
						...
						stripe_id=stripe_card_response.stripe_id,
